...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .charts-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1rem; margin-top: 1.5rem; }
        .chart-card {
            background: rgba(30, 41, 59, 0.9); border: 1px solid #475569; border-radius: 12px;
            padding: 1rem; transition: all 0.3s ease; backdrop-filter: blur(20px);
...
# Generate cards for each chart
for chart in charts:
    install_name = chart['name'].replace('setup-', '').replace('helper-', '')
